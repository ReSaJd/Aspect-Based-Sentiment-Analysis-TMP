...
gloveText = os.path.join('..','utility', 'glove.6B.300d.txt')
useGlove = False
...
# every prepare_data set is a dictionary with entries 'sentences' and 'categories':
...
'read test and training prepare_data from file'
def read():
    global train, test

    trees = [None]*2

    dataPath = os.path.join('..','prepare_data')
    #trees[0] = ET.parse(os.path.join(dataPath,'laptops_train_purified.xml'))
    trees[0] = ET.parse(os.path.join(dataPath,'restaurants_train_purified.xml'))
    #trees[2] = ET.parse(os.path.join(dataPath,'laptops-trial.xml'))
    trees[1] = ET.parse(os.path.join(dataPath,'restaurants-trial.xml'))
    ...
    for i in range(len(trees)):
        if i == 0:
            ...
            test[i-1] = {}
            dataSet = test[i-1]
            dataSet['categories'] = train[i-1]['categories'] # a test set needs to have the same categories as its training set
...
'print training and test prepare_data (at least first n entries)'
def printData(n):
    global train, test
