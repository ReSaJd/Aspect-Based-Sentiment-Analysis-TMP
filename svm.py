...
import parameters
from aspect_category_polarity_feature import AspectCategoryPolarityFeatures
...
'To save all the features as txt'
def sparseVectorToString(vector, featureName):
    ret = ''
    FEATURE_FILE = path.join(TMP_FOLDER, featureName + ".txt")
    for (index,value) in vector:
        ret += ' ' + str(index) + ':' + str(value)
    featureFile = open(FEATURE_FILE, "w")
...
class SVM:
    ...
    def predict(self, test, featureExtractor, resultKey):
        ...
        if self.classifier == 0:
            ...
            if isinstance(featureExtractor, AspectCategoryPolarityFeatures):
